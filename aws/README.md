...
```bash
# Build and run the AWS CLI container
cd aws/
docker-compose -f docker-compose.aws.yml run --rm aws-cli

# Inside the container, run setup
bash setup.sh
```

### Option 2: Direct Execution (Requires AWS CLI Installed)

```bash
# Run with defaults from .env file
./aws/setup.sh

# Or customize with environment variables
AWS_REGION=us-east-1 AWS_ENV=production ./aws/setup.sh

# Non-interactive mode (for CI/CD)
NON_INTERACTIVE=true ./aws/setup.sh

# Use a custom env file
ENV_FILE=/path/to/custom.env ./aws/setup.sh
```
...
|----------|-------------|---------|---------|
| `ENV_FILE` | Env file path | `aws/.env` | `/tmp/aws-prod.env` |
...
**Usage:**
```bash
# Interactive mode using aws/.env
./aws/setup.sh

# Non-interactive mode (CI/CD)
NON_INTERACTIVE=true ./aws/setup.sh

# Override region (interactive)
AWS_REGION=us-east-1 ./aws/setup.sh

# Override environment loaded from aws/.env
AWS_ENV=production ./aws/setup.sh

# Override multiple variables
AWS_REGION=eu-west-1 AWS_ENV=production AWS_PROJECT_ID=uh-groupings-api ./aws/setup.sh

# Use a custom env file
ENV_FILE=/tmp/aws-prod.env ./aws/setup.sh

# Override multiple variables (non-interactive)
NON_INTERACTIVE=true AWS_REGION=eu-west-1 AWS_ENV=production ./aws/setup.sh
